...
async function fetchList() {
  loading.value = true
  try {
    const data = await getAdminLocations(queryParams.value)
...
const dialogTitle = computed(() => (isEdit.value ? '编辑地点' : '新增地点'))
...
async function handleSubmit() {
  ...
  try {
    ...
    if (isEdit.value) {
      await updateAdminLocation(formData.id, payload)
...
async function handleToggleStatus(row) {
  ...
  await ElMessageBox.confirm(`确定要${actionText}该地点吗？`, '提示', {
    type: 'warning',
    confirmButtonText: '确定',
    cancelButtonText: '取消',
  })
  await updateAdminLocation(row.id, { status: nextStatus })
  ElMessage.success('操作成功')
  fetchList()
}

async function handleDelete(row) {
  await ElMessageBox.confirm('确定要删除该地点吗？删除后不可恢复。', '提示', {
    type: 'warning',
...
<template>
  <el-card class="page-index">
    ...
    <el-table :data="list" v-loading="loading">
      ...
      <el-table-column prop="name" label="地点名称" min-width="280" align="center" show-overflow-tooltip />
  ...
  <el-dialog v-model="dialogVisible" :title="dialogTitle" width="420px">
    <el-form ref="formRef" label-width="90px">
      <el-form-item label="地点名称">
        <el-input v-model="formData.name" placeholder="请输入地点名称" maxlength="100" show-word-limit />
      </el-form-item>
